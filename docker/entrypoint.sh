...
if [[ ! -z "$ACLOUDBANKD_SEED" ]]; then
    ARGS+=" --user-provided-seed=$ACLOUDBANKD_SEED"
fi
...
if [[ ! -z "$ACLOUDBANKD_PLUGINS" ]]; then
   exec "$ACLOUDBANKD" --data-dir "${HOME}" ${ARGS} ${ACLOUDBANKD_ARGS} --plugins "${ACLOUDBANKD_PLUGINS}"
else
   exec "$ACLOUDBANKD" --data-dir "${HOME}" ${ARGS} ${ACLOUDBANKD_ARGS}
fi
